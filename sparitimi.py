...
## Fall sem kallad er a ur vidmoti til thess að fa upp hversu mikid tharf ad leggja inn til thess ad ná uppi tiltekna upphaed á tilteknum tima.
...
def manadalegurSparTimi(L, v, vt, heild):
	...
	for i in range(0,10000):
		...
		## Ef summan er orðin hærri en markmidsupphaed, tha skilum vid manudum
		if summa >= heild:
			manAr = manudirIAr(i)
			if vt == 0:
		...
		## Bætum í fylkin fyrir verðbætur og vextir 
...
def eingreidslaSparTimi(L, v, vt, heild):
	...
	for i in range(0, 10000):
		...
		## Ef summan er orðin hærri en markmidsupphaedin tha skilum vid manudunum
		if summa >= heild:
			manAr = manudirIAr(i)
			if vt == 0:
				return "Þú þarft að spara í " + str(manAr) + " til þess að ná uppí " + str(heild) + " kr."
		...
		## Bætum í fylkin fyrir verðbætur og vexti
		summa = summa * (1+(vt/12))
		verdbaeturAr.append(summa*(vt/12))
		vextirAr.append(summa * (v/12))
		if (i+1)%12 == 0 and i != 0:
